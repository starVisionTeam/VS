...
from lib.common.train_util import init_loss, Format, remesh
...
from Mr.test import run_test
from pifuhd_ori.apps.simple_test import feature_capture
...
if __name__ == "__main__":
    ...
    parser.add_argument("-in_dir", "--in_dir", type=str,
                        default="/media/amax/4C76448F76447C28/LYH/thuman2.0/pymafx_test")
    ...
    print(colored(f"Initialize with {Format.start} SMPL-X {Format.end}", "green"))
    ...
    for data in pbar:
        ...
        smpl_verts, smpl_faces = batch_smpl_verts.squeeze(0), batch_smpl_faces.squeeze(0)
        vsb_mask = Visible(smpl_verts, smpl_faces)
        from Mr.morph_pytorch import morph_image, shift

        T_normal_F = F.interpolate(in_tensor['T_normal_F'], (128, 128), mode='bilinear', align_corners=False)
        T_normal_B = F.interpolate(in_tensor['T_normal_B'], (128, 128), mode='bilinear', align_corners=False)
        normal_F = F.interpolate(in_tensor['normal_F'], (128, 128), mode='bilinear', align_corners=False)
        normal_B = F.interpolate(in_tensor['normal_B'], (128, 128), mode='bilinear', align_corners=False)

        f_shift = morph_image(T_normal_F, normal_F)
        b_shift = morph_image(T_normal_B, normal_B)
        strechvs = shift(f_shift, b_shift, smpl_verts, smpl_faces, vsb_mask)
        strechvs_path = os.path.join(args.out_dir, 'VS/obj', data['name'] + '_strech.obj')
        strechvs.export(strechvs_path)
        torch.cuda.empty_cache()

        with torch.no_grad():
            feature, feature_fine = feature_capture(in_tensor['image'], in_tensor["normal_F"], in_tensor["normal_B"])
            torch.cuda.empty_cache()
            recon_obj = run_test(strechvs_path, in_tensor["normal_F"], in_tensor["normal_B"], feature_fine, feature,
                                 vsb_mask, os.path.join(args.out_dir, 'VS/obj', data['name'] + '_recon.obj'))
            torch.cuda.empty_cache()
        ...
        with torch.no_grad():
            per_loop_lst = []
            rotate_recon_lst = dataset.render.get_image(cam_type="four", type='rgb')
        ...
        if args.loop_cloth > 0:
            ...
            for i in loop_cloth:
                ...
                for k in losses.keys():
                    if k not in ["normal", "silhouette", "joint"] and losses[k]["weight"] > 0.0:
            ...
            our = trimesh.load(f"{args.out_dir}/{cfg.name}/obj/{data['name']}_refine.obj", skip_materials=True,
                               process=False, maintain_order=True, force='mesh')
            mesh = trimesh.load(smpl_obj_path, skip_materials=True, process=False, maintain_order=True, force='mesh')
            hand_mesh = mesh.copy()
            smplx_mesh = mesh.copy()
            face_mesh = mesh.copy()
            ...
            our = part_removal(our, face_mesh, 4e-2, device, smplx_mesh, region="face")
            full_lst += [face_mesh]

            our = part_removal(our, hand_mesh, 6e-2, device, smplx_mesh, region="hand")
            full_lst += [hand_mesh]

            full_lst += [our]
            final_path = f"{args.out_dir}/{cfg.name}/obj/{data['name']}_replace.obj"

            final_mesh = poisson(sum(full_lst), final_path, 10)
