...
def poisson(mesh, obj_path, depth=10, face_count=80000, laplacian_remeshing=False):
...
def poisson_laplacian(mesh, obj_path, depth=10, face_count=50000, laplacian_remeshing=True):
    # pcd_path = obj_path[:-4] + "_soups.ply"
    # assert (mesh.vertex_normals.shape[1] == 3)
    # mesh.export(pcd_path)
    # pcl = o3d.io.read_point_cloud(pcd_path)
    ...
    # low_res_mesh = largest_mesh.simplify_quadratic_decimation(face_count)
    if laplacian_remeshing:
        low_res_mesh = trimesh.smoothing.filter_mut_dif_laplacian(mesh, lamb=1.0, iterations=50)
        # low_res_mesh = trimesh.smoothing.filter_humphrey(mesh, alpha=0.1, beta=0.5, iterations=50, laplacian_operator=None)
        # low_res_mesh = trimesh.smoothing.filter_taubin(mesh,lamb=0.9,nu=0.9, iterations=1000)
    low_res_mesh.export(obj_path)
