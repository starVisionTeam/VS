...
class TestDataset:
    ...
    def __getitem__(self, index):
        ...
        if self.hps_type == "pymafx":
            ...
            fit_file = f'/media/amax/4C76448F76447C28/LYH/realworld/extreme/input/pymafx/smplx/%s_f%d_p0.pkl' % (
                img_name[:4], int(img_name[:4]))
            param = torch.load(fit_file)
            param = param[0]
            ...
            param['body_pose'][8, 0, :] = torch.tensor([0.9833, -0.999, 0.9427]).cuda()
            arr_dict['body_pose'] = param['body_pose'][1:22][None]
            # arr_dict['body_pose'][:, 5, :, :] = torch.tensor([[0.9833, -0.999, 0.9427],[0.1793, -0.4700, -0.8643], [-0.0303, 0.8755, -0.4823]]).cuda()
            # arr_dict['body_pose'][:, 4, 0, :] = torch.tensor([0.9833, -1.999, 0.9427]).cuda()
            arr_dict['global_orient'] = param['body_pose'][:1][None]
            arr_dict['left_hand_pose'] = param['left_hand_pose'][None]
            arr_dict['right_hand_pose'] = param['right_hand_pose'][None]
            arr_dict['leye_pose'] = param['leye_pose'][None]
            arr_dict['reye_pose'] = param['reye_pose'][None]
            arr_dict['jaw_pose'] = param['jaw_pose'][None]
