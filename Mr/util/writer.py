...
class Writer:
    ...
    def print_current_losses(self, epoch, i, losses, model_id, view_id, t, t_data):
        """ prints train loss to terminal / file """
        message = '(epoch: %d, iters: %d, time: %.3f, data: %.3f) loss: %.5f moderl_id: %04d view_id: %04d ' \
                  % (epoch, i, t, t_data, losses, model_id, view_id)
        print(message)
        with open(self.log_name, "a") as log_file:
            log_file.write('%s\n' % message)
