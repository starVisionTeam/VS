...
from Mr.util.util import load_obj, export
import copy
...
class Mesh:
    def __init__(self, file, hold_history=False, vs=None, faces=None, device='cpu', gfmm=False):
        ...
        if vs is not None and faces is not None:
            ...
            # tmesh = trimesh.load(file)
            # self.vs = tmesh.vertices
    ...
    def build_gemm(self):
        ...
        for face_id, face in enumerate(self.faces):
            ...
            for idx, edge in enumerate(faces_edges):
                ...
                if edge not in edge2key:
                    ...
                    self.ve[edge[0]].append(edges_count)  # value: verts to edge index: verts index
                    self.ve[edge[1]].append(edges_count)
                    self.vei[edge[0]].append(0)  # 当前点作为边（a,b）的前面a点
                    self.vei[edge[1]].append(1)  # 当前点作为边（a,b）的后面b点
                    nb_count.append(0)
                    edges_count += 1
            for idx, edge in enumerate(faces_edges):
                edge_key = edge2key[edge]
                # if nb_count[edge_key] == 4:
                #     print(edge_key, self.ve[])
        ...
        # for i in range(0,self.gemm_edges.shape[0]):
        #     for j in range(0,self.gemm_edges.shape[1]):
        #         if self.gemm_edges[i,j] == -1:
        #             print('error:value is -1 ----------------------------------',i,j)
        self.sides = np.array(sides, dtype=np.int64)
        self.edges_count = edges_count
        # lots of DS for loss
        self.nvs, self.nvsi, self.nvsin = [], [], []
        for i, e in enumerate(self.ve):
            self.nvs.append(len(e))  # 每个点连接边的个数
            self.nvsi.append(len(e) * [i])
            self.nvsin.append(list(range(len(e))))
        self.vei = torch.Tensor(np.concatenate(np.array(self.vei)).ravel()).to(self.device).long()
    ...
    def normalize_unit_bb(self):
        ...
        scale = max([self.vs[:, i].max() - self.vs[:, i].min() for i in range(3)])
        self.vs = self.vs / scale
        target_mins = [(self.vs[:, i].max() - self.vs[:, i].min()) / -2.0 for i in range(3)]
        translations = [(target_mins[i] - self.vs[:, i].min()) for i in range(3)]
        self.vs += np.array(translations)[None, :]
        self.scale = scale
        self.translations = np.array(translations)[None, :]
    ...
    def discrete_project(self, pc: torch.Tensor, thres=0.9, cpu=False):
        with torch.no_grad():
            ...
            if isinstance(self, Mesh):
                mid_points = self.vs[self.faces].mean(dim=1).to('cuda')
            ...
            knn_mask = (loop == torch.arange(0, pk12.shape[0], device=torch.device('cuda'))[:, None]).sum(dim=1) > 0
...
class PartMesh:
    ...
    def __init__(self, main_mesh: Mesh, vs_groups=None, num_parts=1, bfs_depth=0, n=-1):
        ...
        self.sub_mesh_edge_index = []  # 里面为submesh中边的索引
        for i in range(self.n_submeshes):
    ...
    @staticmethod  # 以center为中心分成八个区域，类似xyz坐标系的八个不同区域
    def segment_shape(vs: torch.Tensor, seg_num: int):
        ...
        diff = vs - center[None, :]  # 不懂为什么加none
