...
def chamfer_distance1(
    ...
    if return_normals:
        ...
        # cham_norm_x = 1-1 * cham_norm_x
        # cham_norm_y = 1-1 * cham_norm_y
...
class BeamGapLoss:
    ...
    def update_pm(self, pmesh, target_pc):
        ...
        for i, m in enumerate(pmesh):
            p, mask = m.discrete_project(target_pc, thres=0.99, cpu=False)
    ...
    def __call__(self, pmesh, j):
        losses = self.points[j] - pmesh[j].vs[pmesh[j].faces].mean(dim=1).to('cuda')
