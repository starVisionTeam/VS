...
class HGPIFuNetwNML(BasePIFuNet):
    ...
    def filter(self, images, norm_F, norm_B):
        '''
        ...
        images = torch.cat([images, norm_F, norm_B], 1)

        self.im_feat_list, self.normx = self.image_filter(images)

        if not self.training:
            self.im_feat_list = [self.im_feat_list[-1]]  ###########add
            feature = self.im_feat_list[-1]
            # feature = self.im_feat_list[-1].squeeze(0)
            # feature = feature.detach().cpu().numpy()
            # os.makedirs('/media/amax/4C76448F76447C28/LYH/realworld/extreme/feature_hd', exist_ok=True)
            # np.save('/media/amax/4C76448F76447C28/LYH/realworld/extreme/feature_hd/%s' % (name), feature)
        return feature
