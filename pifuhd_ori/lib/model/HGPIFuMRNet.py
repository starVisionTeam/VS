...
class HGPIFuMRNet(BasePIFuNet):
    ...
    def filter_global(self, images, norm_F, norm_B):
        '''
        ...
        if self.opt.train_full_pifu:
            ...
            with torch.no_grad():
                feature = self.netG.filter(images, norm_F, norm_B)
        return feature

    def filter_local(self, images, norm_F, norm_B, rect=None):
        '''
        apply a fully convolutional network to images.
        the resulting feature will be stored.
        args:
        ...
        if len(nmls):
            nmls = nn.Upsample(size=(self.opt.loadSizeBig, self.opt.loadSizeBig), mode='bilinear', align_corners=True)(
                torch.cat(nmls, 1))
            ...
            if rect is None:
                ...
                # images = torch.cat([images, normals_f, normals_b], 1)
                images = torch.cat([images, nml], 2)

        self.im_feat_list, self.normx = self.image_filter(images.view(-1, *images.size()[2:]))  ###############is list
        feature = self.im_feat_list[-1]
        return feature
        # feature = self.im_feat_list[-1].squeeze(0)
        # feature = feature.detach().cpu().numpy()
        # os.makedirs('/media/amax/4C76448F76447C28/LYH/realworld/extreme/feature_hd_fine', exist_ok=True)
        # np.save('/media/amax/4C76448F76447C28/LYH/realworld/extreme/feature_hd_fine/%s' % name, feature)
        ...
        for i in range(B):
            ...
            if labels is not None:
                ...
                with torch.no_grad():
                    ...
                    gammas.append(
                        1 - newlabels[-1].view(newlabels[-1].size(0), -1).sum(1) / in_bb.view(in_bb.size(0), -1).sum(1))
    ...
    def calc_normal(self, points, calib_local, calib_global, transforms=None, labels=None, delta=0.001,
                    fd_type='forward'):
        '''
    ...
    def forward(self, images_local, images_global, points, calib_local, calib_global, labels, points_nml=None,
                labels_nml=None, rect=None):
